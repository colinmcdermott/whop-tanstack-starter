...
const getSuccessData = createServerFn({ method: "GET" }).handler(async () => {
  const [session, plans] = await Promise.all([getSession(), getPlansConfig()]);
  return { isSignedIn: !!session, plans };
});
...
export const Route = createFileRoute("/checkout/success")({
  ...
  loader: () => getSuccessData(),
  head: () => ({
    meta: [{ title: "Welcome | Whop SaaS Starter" }],
  }),
...
function CheckoutSuccessPage() {
  ...
  const { isSignedIn, plans } = Route.useLoaderData();
  ...
  const planName = planKey ? (plans[planKey]?.name ?? null) : null;
